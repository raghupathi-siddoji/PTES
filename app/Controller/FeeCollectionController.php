...
class FeeCollectionController extends AppController {
  ...
  public  function getStudents(){
        ...
        if(isset($this->request['data']['class_id'])) {
           if($this->request['data']['other_medium']==null) {
                $condition = array(
                    'StudentDetail.add_class_id' => $this->request['data']['class_id'],
                    'StudentDetail.academic_year_id' => $this->request['data']['year'],
                ...
                $condition = array(
                    'StudentDetail.add_class_id' => $this->request['data']['class_id'],
                    'StudentDetail.academic_year_id' => $this->request['data']['year'],
                    'StudentDetail.other_medium' => $this->request['data']['other_medium'],

                );
            }

            $studentDetail = $this->StudentDetail->find('all', array( 'order'=>'student_name',
                'fields' => array(
                    'id',
                    'student_name',
                    'student_code'
            ...
            $list_array = array();
			foreach($studentDetail as $slist){
					$list_array[$slist['StudentDetail']['student_name']."-".$slist['StudentDetail']['student_code']] =  $slist['StudentDetail']['student_name']."-".$slist['StudentDetail']['student_code'];
			} 
			$student1='"'.implode('","',$list_array).'"'; 
        }
