...
/*	public function _userSessionCheckout()
	...
	public function _selectDropDown()
		...
		public function _SmsDropDown()
	{
		/* Get Notification Type Dropdown Select List */
		$notification_list=$this->NotificationType->find('all',array('order'=>'NotificationType.notification_type DESC'));
		$notification_array = array();
		if(!empty($notification_list))
		{
			foreach($notification_list as $not_list)
			{ 
				$notification_array[$not_list['NotificationType']['notification_type']] = $not_list['NotificationType']['notification_type'];
			}
		}
		$this->set('notification_list',$notification_array);
	...
	function _getClass()
    {
        ...
        //kinder class
        $class_array_kinder = array();
        if(!empty($class_list))
        {
            foreach($class_list as $class)
            {
                if($class['AddClass']['class']=='PreNursery' || $class['AddClass']['class']=='LKG'|| $class['AddClass']['class']=='UKG')
                    $class_array_kinder[$class['AddClass']['id']] = $class['AddClass']['class'];
            }
        }
        $this->set('classesKinder',$class_array_kinder);
        // Higher  class
        $class_array = array();
        if(!empty($class_list))
        {
	...
	public function _getAcademicYear()
		{
			/* Get Academic Year Dropdown Select List */
			$academic_year_list=$this->AcademicYear->find('all',array('order'=>'AcademicYear.academic_year DESC'));
			$academic_array = array();
